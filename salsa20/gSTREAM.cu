...
void gSTREAM_init(gSTREAM_ctx* ctx, int device, int nr_threads, int nr_blocks){
   ...
   Salsa20_ctx *sctx=&ctx->sctx;
   cutilSafeCall(cudaMalloc((void**)&(sctx->x_d),nr_streams*16*sizeof(u32)));
...
void gSTREAM_exit(gSTREAM_ctx* ctx) {
   ...
   /* free cipher state */
   Salsa20_ctx *sctx=&ctx->sctx;
   cutilSafeCall(cudaFree(sctx->x_d));
}
...
void gSTREAM_keysetup(gSTREAM_ctx* ctx, u8* keys, u32 keysize, u32 ivsize) {
   ...
   free(keys_h);
   if(ctx->iv_size<=0) { /* should not really happend */
      Salsa20_ctx *sctx=&ctx->sctx;
      Salsa20_keyivsetup<<<ctx->nr_blocks,ctx->nr_threads>>>(sctx->x_d
                                                            ,ctx->keys_d
                                                            ,ctx->key_size
                                                            ,ctx->ivs_d
                                                            ,ctx->iv_size);
...
void gSTREAM_ivsetup(gSTREAM_ctx* ctx, u8* ivs) {
   ...
   {//print state, each colum corresponds to a different stream
      u32 *x_h;
      if(!(x_h=(u32*)malloc(nr_streams*16*sizeof(u32)))) {
         fprintf(stderr, "Failed to allocate c_h: %s\n",strerror(errno));
         exit(-1);
      }

      cutilSafeCall(cudaMemcpy(x_h,sctx->x_d,(nr_streams*16*sizeof(u32)),
               cudaMemcpyDeviceToHost));

      for(int i=0;i<nr_streams*16;i++) {
         printf("[0x%08x], ",x_h[i]);
...
void gSTREAM_process_bytes(gSTREAM_action action, gSTREAM_ctx* ctx,
   ...
   size_t length_nr_blocks=(((length-1)/(sizeof(u8)*64))+1);
   size_t length_nr_words=length_nr_blocks*16;
   ...
   Salsa20_process_blocks<<<ctx->nr_blocks,ctx->nr_threads
#ifdef INPUT_SHMEM
      ,ctx->nr_threads*16*sizeof(u32)
#endif
      >>>(action
                                                             ,sctx->x_d
                                                             ,ctx->buff_d
