...
void gSTREAM_init(gSTREAM_ctx* ctx, int device, int nr_threads, int nr_blocks){
   ...
   /* ---------------------------------------------------------------------- */
   /* allocate Trivium state */

   cutilSafeCall(cudaMalloc((void**)&(tctx->s32_d),nr_streams*10*sizeof(u32)));
}
...
void gSTREAM_exit(gSTREAM_ctx* ctx) {
   ...
   cutilSafeCall(cudaFree(tctx->s32_d));
}
...
void gSTREAM_ivsetup(gSTREAM_ctx* ctx, u8* ivs) {
   ...
   Trivium_keyivsetup<<<ctx->nr_blocks,ctx->nr_threads>>>(tctx->s32_d
                                                         ,ctx->keys_d
                                                         ,ctx->key_size
                                                         ,ctx->ivs_d
                                                         ,ctx->iv_size);
   ...
   {//print state, each colum corresponds to a different stream
      u32 *s_h;
      if(!(s_h=(u32*)malloc(nr_streams*10*sizeof(u32)))) {
         fprintf(stderr, "Failed to allocate s_h: %s\n",strerror(errno));
         exit(-1);
      }

      cutilSafeCall(cudaMemcpy(s_h,tctx->s32_d,(nr_streams*10*sizeof(u32)),
               cudaMemcpyDeviceToHost));

      for(int i=0;i<nr_streams*10;i++) {
         printf("[0x%08x], ",s_h[i]);
         if(!((i+1)%nr_streams)) { printf("\n"); }
      }
...
void gSTREAM_process_bytes(gSTREAM_action action, gSTREAM_ctx* ctx,
   ...
   Trivium_process_bytes<<<ctx->nr_blocks,ctx->nr_threads>>>(action
                                                            ,tctx->s32_d
                                                            ,ctx->buff_d
                                                            ,length_nr_words);
   cutilCheckMsg("Kernel execution failed");
