...
void gSTREAM_init(gSTREAM_ctx* ctx, int device, int nr_threads, int nr_blocks){
   ...
   Rabbit_ctx *rctx=&ctx->rctx;
   cutilSafeCall(cudaMalloc((void**)&(rctx->x_d),nr_streams*8*sizeof(u32)));
   cutilSafeCall(cudaMalloc((void**)&(rctx->c_d),nr_streams*8*sizeof(u32)));
   cutilSafeCall(cudaMalloc((void**)&(rctx->carry_d),nr_streams*sizeof(u32)));
...
void gSTREAM_exit(gSTREAM_ctx* ctx) {
   ...
   /* free cipher state */
   Rabbit_ctx *rctx=&ctx->rctx;
   cutilSafeCall(cudaFree(rctx->x_d));
   cutilSafeCall(cudaFree(rctx->c_d));
   cutilSafeCall(cudaFree(rctx->carry_d));
}
...
void gSTREAM_keysetup(gSTREAM_ctx* ctx, u8* keys, u32 keysize, u32 ivsize) {
   ...
   Rabbit_ctx *rctx=&ctx->rctx;
   Rabbit_keysetup<<<ctx->nr_blocks,ctx->nr_threads>>>(rctx->x_d
                                                      ,rctx->c_d
                                                      ,rctx->carry_d
                                                      ,ctx->keys_d
                                                      ,ctx->key_size);
   ...
   {//print state, each colum corresponds to a different stream
      u32 *x_h, *c_h,*carry_h;
      if(!(x_h=(u32*)malloc(nr_streams*8*sizeof(u32)))) {
         fprintf(stderr, "Failed to allocate c_h: %s\n",strerror(errno));
         exit(-1);
      }
      if(!(c_h=(u32*)malloc(nr_streams*8*sizeof(u32)))) {
         fprintf(stderr, "Failed to allocate c_h: %s\n",strerror(errno));
         exit(-1);
      }
      if(!(carry_h=(u32*)malloc(nr_streams*sizeof(u32)))) {
         fprintf(stderr, "Failed to allocate carry_h: %s\n",strerror(errno));
         exit(-1);
      }

      cutilSafeCall(cudaMemcpy(x_h,rctx->x_d,(nr_streams*8*sizeof(u32)),
               cudaMemcpyDeviceToHost));
      cutilSafeCall(cudaMemcpy(c_h,rctx->c_d,(nr_streams*8*sizeof(u32)),
               cudaMemcpyDeviceToHost));
      cutilSafeCall(cudaMemcpy(carry_h,rctx->carry_d,(nr_streams*sizeof(u32)),
               cudaMemcpyDeviceToHost));

      for(int i=0;i<nr_streams*8;i++) {
         printf("[0x%08x:0x%08x], ",x_h[i],c_h[i]);
         if(!((i+1)%nr_streams)) { printf("\n"); }
      }
      for(int i=0;i<nr_streams;i++) {
         printf("[     0x%08x      ], ",carry_h[i]);
      }
...
void gSTREAM_ivsetup(gSTREAM_ctx* ctx, u8* ivs) {
   ...
   Rabbit_ctx *rctx=&ctx->rctx;
   Rabbit_ivsetup<<<ctx->nr_blocks,ctx->nr_threads>>>(rctx->x_d
                                                     ,rctx->c_d
                                                     ,rctx->carry_d
                                                     ,ctx->ivs_d
                                                     ,ctx->iv_size);
...
void gSTREAM_process_bytes(gSTREAM_action action, gSTREAM_ctx* ctx,
   ...
   Rabbit_process_bytes<<<ctx->nr_blocks,ctx->nr_threads>>>(action
                                                           ,rctx->x_d
                                                           ,rctx->c_d
                                                           ,rctx->carry_d
                                                           ,ctx->buff_d
